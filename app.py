...
def order():
    if request.method == 'POST':
        ...
        if table and menu_item:
            ...
            new_order = Order(user_id=1, table_id=table.id, total=total)  # Assuming user_id=1 for simplicity
...
def reservation():
    if request.method == 'POST':
        ...
        if table and not table.is_occupied:
            new_reservation = Reservation(table_id=table.id, customer_name=customer_name, reservation_time=datetime.strptime(reservation_time, '%Y-%m-%dT%H:%M'))
